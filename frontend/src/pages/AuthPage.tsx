...
import { Brain, GraduationCap, Users, ArrowRight, Lock, Mail, User, Moon, Sun, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    if (urlError) {
      ...
      if (urlError === 'oauth_failed') msg = 'Google authentication failed. Please try again.';
      else if (urlError === 'invalid_role') msg = 'You already have an account with a different role. Please select the correct role.';
      else if (urlError === 'missing_code') msg = 'Missing authorization code from Google.';
      else if (urlError === 'invalid_callback') msg = 'Invalid authentication callback.';
      else msg = `Error: ${urlError}`;
      setError(msg);
    }
  }, [searchParams]);
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        localStorage.setItem('eduxcel_user', JSON.stringify(u));
        handleAuthSuccess(u);
      } catch (err: any) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: bg, transition: 'background 0.3s', position: 'relative', overflow: 'auto' }}>
      ...
      <button
        onClick={toggleTheme}
        style={{
          position: 'fixed', top: 16, right: 16, zIndex: 100, padding: 10, borderRadius: '50%',
          background: surface, border: `1px solid ${border}`, cursor: 'pointer', display: 'flex',
        ...
        <div style={{ width: '100%', maxWidth: 440, position: 'relative', zIndex: 1 }}>
          ...
          <motion.div
            ...
            style={{
              ...
              boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.4)' : '0 8px 32px rgba(11,30,74,0.08)',
            }}
          >
            ...
            {error && (
              <div
                style={{
                  ...
                  color: isDark ? '#f87171' : '#dc2626', display: 'flex', alignItems: 'center', gap: 8,
                }}
              >
            ...
            <div style={{ display: 'flex', background: isDark ? '#21262d' : '#f1f5f9', borderRadius: 10, padding: 4, marginBottom: 24 }}>
              {(['student', 'faculty', 'admin'] as Role[]).map((r) => (
                <button
                  ...
                  onClick={() => setRole(r)}
                  style={{
                    flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 4px',
            ...
            <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button
                ...
                disabled={loading}
                style={{
                  width: '100%', padding: '12px', background: '#0047BA', color: '#fff', border: 'none', borderRadius: 10,
                  fontWeight: 700, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                  opacity: loading ? 0.7 : 1, boxShadow: '0 4px 16px rgba(0,71,186,0.3)', transition: 'background 0.2s', marginTop: 4,
                }}
              ...
              <button
                ...
                style={{
                  width: '100%', padding: '12px', background: surface, color: text, border: `1px solid ${border}`,
                  borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: googleLoading ? 'not-allowed' : 'pointer',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 12, transition: 'background 0.2s',
                }}
                onMouseEnter={(e) => !googleLoading && (e.currentTarget.style.background = isDark ? '#21262d' : '#f1f5f9')}
            ...
            <div style={{ marginTop: 16, padding: '12px 16px', background: isDark ? 'rgba(53,103,251,0.08)' : '#eef3ff', border: `1px solid ${isDark ? 'rgba(53,103,251,0.2)' : '#c6d6ff'}`, borderRadius: 12, fontSize: 12, color: isDark ? '#8aaaff' : '#0047BA', textAlign: 'center' }}>
              <CheckCircle style={{ width: 14, height: 14, display: 'inline', marginRight: 4 }} />
